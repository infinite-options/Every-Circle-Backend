...
class HandleAdditionalBusinessUsersTests(unittest.TestCase):
    ...
    def test_email_without_account_is_reported_not_saved(self):
        db = MagicMock()
        db.select.side_effect = _select_router(users_by_email={})  # nobody matches
        summary = self.resource._handle_additional_business_users(
            db, "200-biz", ["ghost@example.com"], ["employee"], exclude_user_id=None
        )
    ...
    def test_new_known_user_is_inserted_and_reported(self):
        ...
        summary = self.resource._handle_additional_business_users(
            db, "200-biz", ["alex@example.com"], ["employee"], exclude_user_id="100-owner"
        )
    ...
    def test_length_mismatch_processes_overlap_and_flags(self):
        ...
        db.call.return_value = {"result": [{"new_id": "700-new"}]}
        summary = self.resource._handle_additional_business_users(
            db, "200-biz", ["alex@example.com", "second@example.com"], ["employee"], exclude_user_id=None
        )
