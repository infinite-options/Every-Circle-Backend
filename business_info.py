...
class BusinessInfo(Resource):
        ...
        try:
            with connect() as db:
                ...
                if uid.startswith("200"):
                    ...
                    # Google ID lookup
                    business_query = db.select(
                        "every_circle.business", where={"business_google_id": uid}
                    )
                    if business_query["result"]:
                ...
                response = {
                    ...
                    "categories": (
                        category_response["result"]
                        if "result" in category_response
                        else []
                    ),
                    "social_links": (
        ...
        try:
            ...
            with connect() as db:
                ...
                if not business_google_id and not business_name:
                    response["message"] = (
                        "business_google_id or business_name is required"
                    )
                    response["code"] = 400
                    return response, 400
    ...
    def put(self):
        ...
        try:
            ...
            with connect() as db:
                ...
                if business_role is not None and business_user_id is not None:
                    ...
                    if business_user_query["result"]:
                        ...
                        if existing_role != business_role:
                            ...
                            print(
                                f"Updated business_user role from '{existing_role}' to '{business_role}' for business {business_uid}, user {business_user_id}"
                            )
                        else:
                            print(
                                f"Business_user role unchanged: '{business_role}' for business {business_uid}, user {business_user_id}"
                            )
                    else:
                        ...
                        try:
                            ...
                            if (
                                ...
                                response["message"] = (
                                    f'Failed to generate bu_uid: {bu_uid_response.get("message", "Unknown error")}'
                                )
                                response["code"] = 500
                                return response, 500
                            ...
                            print(
                                f"Inserted new business_user record for business {business_uid}, user {business_user_id}, role {business_role}"
                            )
                        except Exception as e:
                            print(
                ...
                business_users_individual_public = payload.pop(
                    "business_users_individual_public", None
                )
                            ...
                            for item in public_list:
                                ...
                                if bu_row.get("result"):
                                    ...
                                    db.update(
                                        ...
                                        {"bu_uid": bu_uid},
                                        {
                                            "bu_individual_business_is_public": (
                                                1 if is_public else 0
                                            )
                                        },
                                    )
                                    print(
                                        f"Updated bu_individual_business_is_public={1 if is_public else 0} for business {business_uid}, user {bu_user_id}"
                                    )
                    ...
                    try:
                        ...
                        if custom_tags is not None:
                            if not isinstance(custom_tags, list):
                                ...
                                print(
                                    f"Deleted existing business_tags for business {business_uid}"
                                )

                                # Process and add new tags
                                stored_tags = self._process_tags(
                                    db, business_uid, custom_tags
                                )
                                response["updated_tags"] = stored_tags
                                print(
                                    f"Processed {len(stored_tags)} tags for business {business_uid}"
                                )
                    except json.JSONDecodeError as e:
                        print(f"Error parsing custom_tags JSON: {str(e)}")
                    except Exception as e:
                    ...
                    try:
                        ...
                        for service_data in services_data:
                            ...
                            if "bs_uid" in service_data:
                                ...
                                service_exists_query = db.select(
                                ...
                                if "bs_bounty" in service_data:
                                    service_data["bs_bounty"] = _strip_currency(
                                        service_data["bs_bounty"]
                                    )
                                if "bs_bounty_type" not in service_data:
                                    service_data["bs_bounty_type"] = "per_item"
                                ...
                                if service_user_id:
                                    ...
                                    print(
                                        "Warning: Cannot add service - service_user_id is None"
                                    )

                    except Exception as e:
                        print(
                            f"Error processing business_services JSON in PUT: {str(e)}"
                        )
                ...
                if (
                    ...
                    print(
                        "[BUSINESS PROFILE IMAGE] PUT - request.files keys=%s"
                        % (list(request.files.keys()) if request.files else [])
                    )
                    print(
                        "[BUSINESS PROFILE IMAGE] PUT - business_profile_img in request.files=%s, delete_business_profile_img in payload=%s"
                        % (
                            "business_profile_img" in (request.files or {}),
                            "delete_business_profile_img" in payload,
                        )
                    ...
                    if payload.get("delete_business_profile_img"):
                        print(
                            "[BUSINESS PROFILE IMAGE] PUT - delete_business_profile_img value=%s"
                            % (
                                (payload["delete_business_profile_img"] or "")[:80]
                                + (
                                    "..."
                    ...
                    payload["business_profile_img"] = result
                    print(
                        "[BUSINESS PROFILE IMAGE] PUT - URL stored in DB (business_profile_img)=%s"
                        % (result)
                    )
                ...
                invalid_fields = [
                    field for field in payload.keys() if field not in valid_columns
                ]
                for field in invalid_fields:
                ...
                if business_profile_img_url is not None:
                    ...
                    print(
                        "[BUSINESS PROFILE IMAGE] PUT - Re-applied URL before update (len=%s)"
                        % (len(business_profile_img_url))
                    )

                print("Final payload:", payload)
                print(
                    "[BUSINESS PROFILE IMAGE] PUT - business_profile_img in payload=%s"
                    % ("business_profile_img" in payload)
                )
                ...
                print(
                    "[DB WRITE] About to PUT to database - business_profile_img=%s"
                    % (_url if _url else "(none)")
                )
                print(
                    "[DB WRITE] About to PUT to database - business_profile_img_is_public=%s"
                    % (
                        _is_public
                        if _is_public is not None and _is_public != ""
                        else "(none/empty)"
                    )
                )
                if payload:
    ...
    def _add_social_links(self, db, social_links_str, business_uid):
        try:
            ...
            for social_platform, url in social_links.items():
                social_link_query = db.select(
                    "every_circle.social_link",
    ...
    def _update_social_links(self, db, social_links_str, business_uid):
        try:
            ...
            existing_links = {}
            if (
                "result" in existing_links_response
                and existing_links_response["result"]
            ):
    ...
    def _handle_additional_business_users(
        self,
        db,
        business_uid,
        additional_business_user,
        ...
        try:
            ...
            existing_records_query = db.select(
                "every_circle.business_user", where={"bu_business_id": business_uid}
            )
            existing_records = {}
            ...
            for email, role in zip(additional_business_user, additional_business_role):
                ...
                if business_user_query["result"]:
                    ...
                    try:
                        ...
                        print(
                            f"Exception creating new business_user record for email {email}: {str(e)}"
                        )
                        traceback.print_exc()
        ...
        for tag in tags:
            try:
                ...
                if tag_query.get("result") and len(tag_query["result"]) > 0:
                    ...
                    if (
                        not tag_uid_response.get("result")
                        or len(tag_uid_response["result"]) == 0
                    ):
                        print(
                            f"Warning: Failed to generate tag UID for tag: {tag_name}"
                        )
                        continue

                    tag_uid = tag_uid_response["result"][0]["new_id"]

                    tag_payload = {"tag_uid": tag_uid, "tag_name": tag_name}
                    ...
                    if tag_insert.get("code") != 200:
                        print(
                            f"Failed to create tag {tag_name}: {tag_insert.get('message')}"
                        )
                        continue

                # Check if business-tag association already exists
                existing_bt_query = db.select(
                    "every_circle.business_tags",
                    where={"bt_business_id": business_uid, "bt_tag_id": tag_uid},
                )

                if (
                    existing_bt_query.get("result")
                    and len(existing_bt_query["result"]) > 0
                ):
                    print(
                        f"Business-tag association already exists for tag: {tag_name}"
                    )
                    stored_tags.append({"tag_name": tag_name, "tag_uid": tag_uid})
                    continue

                # Create business-tag association in every_circle.business_tags
                bt_uid_response = db.call(procedure="new_bt_uid")
                if (
                    not bt_uid_response.get("result")
                    or len(bt_uid_response["result"]) == 0
                ):
                    print(
                        f"Warning: Failed to generate business_tag UID for tag: {tag_name}"
                    )
                    continue
                ...
                if bt_response.get("code") == 200:
                    stored_tags.append({"tag_name": tag_name, "tag_uid": tag_uid})
                    print(
                        f"Created business-tag association: {bt_uid} for tag {tag_name}"
                    )
                else:
                    print(
                        f"Failed to create business-tag association: {bt_response.get('message')}"
                    )
        ...
        try:
            ...
            with connect() as db:
                ...
                business_data = {
                    ...
                    "business_latitude": place_data.get("geometry", {})
                    .get("location", {})
                    .get("lat"),
                    "business_longitude": place_data.get("geometry", {})
                    .get("location", {})
                    .get("lng"),
                    "business_joined_timestamp": datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                ...
                # Verify the insert
                verify_query = (
                    "SELECT * FROM every_circle.business WHERE business_uid = %s"
                )
