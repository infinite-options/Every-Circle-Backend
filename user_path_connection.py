...
class ConnectionsPath(Resource):
    def get(self, first_uid, second_uid):
        ...
        with connect() as db:
            ...
            # Try the precomputed profile_personal_path first (fast, indexed lookup)
            first_stored_query = f''' SELECT profile_personal_path AS path
                                FROM profile_personal
                                WHERE profile_personal_uid = '{first_uid}';
                        '''

            response = db.execute(first_stored_query)
            print(response)

            if response['result'] and response['result'][0]['path']:
                stored_path = response['result'][0]['path']
                first_user_details = {'path': '->'.join(uid.strip().strip("'") for uid in stored_path.split(','))}
            else:
                # profile_personal_path not available - fall back to recomputing it
            ...
            # Try the precomputed profile_personal_path first (fast, indexed lookup)
            second_stored_query = f''' SELECT profile_personal_path AS path
                                FROM profile_personal
                                WHERE profile_personal_uid = '{second_uid}';
                        '''

            response = db.execute(second_stored_query)
            print(response)

            if response['result'] and response['result'][0]['path']:
                stored_path = response['result'][0]['path']
                second_user_details = {'path': '->'.join(uid.strip().strip("'") for uid in stored_path.split(','))}
            else:
                # profile_personal_path not available - fall back to recomputing it
